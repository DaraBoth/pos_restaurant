...
export default function ExchangeRateManagement() {
    ...
    async function loadRate() {
        ...
        try {
            const current = await getExchangeRate(restaurantId);
            setRate(current);
    ...
    async function handleUpdate(confirmed = false) {
        ...
        setLoading(true);
        try {
            await setExchangeRate(val, restaurantId);
            await loadRate();
            // Propagate the new rate app-wide so the stale banner, sidebar rate, and
            // order totals update immediately without a reload.
            refreshRate();
        } catch (e) {
            console.error(e);
            alert(t('error'));
